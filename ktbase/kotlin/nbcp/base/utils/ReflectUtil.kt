...
//        }
        ...
        var beforeKeys = unwindKeys.ArraySlice(0, -1);
        var lastKey = unwindKeys.last()
     ...
     */
    private fun getPrivatePropertyValue(
        entity: Any?,
    ...
    private fun setPrivatePropertyValue(
        ...
        if (type != null) {
            ...
            if (properties.size == 1) {
                ...
                var leftProperties = properties.slice(1 until properties.size).toTypedArray();
                return setPrivatePropertyValue(
                    ret,
                    *leftProperties,
                    ignoreCase = ignoreCase,
                    value = value
            ...
            if (data is Array<*>) {
                ...
                if (data.size < index) {
                    if (touchType == WbsNoKeyTouchEnum.Fill) {
                        ...
                        throw RuntimeException("数组 找不到索引 key: ${key}, index: ${index}")
                    }
                }
                ...
                throw RuntimeException(
                    "需要数组类型,但是实际类型是${data::class.java.name}, keys:${
                        keys.joinToString(
                            ","
                        )
                    },key: ${key}"
