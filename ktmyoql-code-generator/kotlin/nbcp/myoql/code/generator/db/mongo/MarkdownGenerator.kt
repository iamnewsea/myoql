...
"""
        ...
        enums.forEach {
            writeToFile(genEnum(it as Class<out Enum<*>>));
        }
...
//        println(msg)
    ...
    fun getEnums(groups: MutableMap<String, out List<Class<*>>>): MutableList<Class<Enum<*>>> {
        var list = mutableListOf<Class<Enum<*>>>()

        groups.values.forEach {
            it.forEach {
                it.findAllEnum().forEach {
    ...
    fun <T : Enum<T>> genEnum(entType: Class<T>): String {
