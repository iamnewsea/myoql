...
val ServerWebExchange.XTraceId: String
    get() {
        var xTraceId = this.request.getHeader("X-Trace_Id");
        if( xTraceId.HasValue){
            return xTraceId;
        }

        xTraceId = this.attributes.get("[XTraceId]").AsString()
        if (xTraceId.HasValue) {
            return xTraceId
        }
        ...
        this.attributes.set("[XTraceId]", xTraceId);
        return xTraceId;
    }
